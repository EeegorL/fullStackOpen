...
const authorWithMostBlogs = (blogs) => {
  ...
  for(const blog of blogs) {
    if(authors[blog.author]) authors[blog.author]++;
    else authors[blog.author] = 1;
  }

  const mostActiveAuthor = Object.keys(authors).reduce((max, n) => authors[n] > authors[max] ? n : max);
  return {
    author: mostActiveAuthor,
    blogs: authors[mostActiveAuthor]
  };
};
...
const authorWithMostLikes = (blogs) => {
  ...
  const mostLikedAuthor = Object.keys(authors).reduce((max, n) => authors[n] > authors[max] ? n : max);
  return {
